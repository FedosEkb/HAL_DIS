...
#define USART_REG_CR1_IDLE_INT_EN                		( (uint32_t)( 1 << 4) )
#define USART_REG_CR1_TE                  			 	( (uint32_t)( 1 << 3) )
#define USART_REG_CR1_RE                  			 	( (uint32_t)( 1 << 2) )

/*******************  Bit definition for USART_CR2 register  ********************/
#define USART_REG_CR2_STOP_BITS                 	 	12   //maybe  ( (uint32_t)( 1 << 12) )??????? it's ok.
...
#define USART_STOP_BITS_MASK                     		(uint32_t)(0b11 << 12)


/*******************  Bit definition for USART_CR3 register  ********************/
#define USART_REG_CR3_ERR_INT_EN                  		( (uint32_t)( 1 << 0) )

#define UART_STOPBITS_1        							( (uint32_t)0x00 ) // repeat ^^
#define UART_STOPBITS_HALF     							( (uint32_t)0x01 ) // repeat ^^
#define UART_STOPBITS_2        							( (uint32_t)0x02 ) // repeat ^^
#define UART_STOPBITS_ONENHALF 							( (uint32_t)0x03 ) // repeat ^^

#define UART_PARITY_NONE                  				((uint32_t)0x00000000) //  CR1 BIT 10 parity control enable
#define UART_HWCONTROL_NONE               				((uint32_t)0x00000000) //  CR3 BIT 10 CTS interrupt enable
														//CR3 BIT 9 CTS enable	CR3 BIT 8 RTS enable it's for initialization structure
